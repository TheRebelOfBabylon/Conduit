...
func TestInitConfigNoYAML(test *testing.T) {
	...
	if utils.FileExists(config_file_path) {
		...
		if err != nil {
			test.Fatalf("Error opening config.yaml: %v", err)
		}
		err = ioutil.WriteFile(config_copy_path, config_file, 0666)
		if err != nil {
			test.Fatalf("Error copying config.yaml to config.copy.yaml: %v", err)
		}
		err = os.Remove(config_file_path)
		if err != nil {
			test.Fatalf("Errror deleting original config.yaml: %v", err)
		}
		defer func() {
			config_copy_file, err := ioutil.ReadFile(config_copy_path)
			if err != nil {
				test.Fatalf("Error opening config.copy.yaml: %v", err)
			}
			err = ioutil.WriteFile(config_file_path, config_copy_file, 0666)
			if err != nil {
				test.Fatalf("Error copying config.copy.yaml to config.yaml: %v", err)
			}
			err = os.Remove(config_copy_path)
			if err != nil {
				test.Fatalf("Errror deleting config.copy.yaml: %v", err)
			}
		}()
	}
...
func TestInitConfigFromYAML(test *testing.T) {
	...
	if utils.FileExists(config_file_path) {
		...
		if err != nil {
			test.Fatalf("Error creating config.yaml: %v", err)
		}
		defer func() {
	...
	if err != nil {
		test.Fatalf("Error creating/opening config.yaml: %v", err)
	}
	// write to yaml file
	d_config := Config{
		DefaultDir:    true,
	...
	if !cmp.Equal(*config, d_config) {
		test.Errorf("InitConfig did not properly read the config file: %v", *config)
	}
}

// TestDefaultDir tests that default_dir returns the expected default directory
func TestDefaultDir(test *testing.T) {
	home_dir := utils.AppDataDir("conduit", false)
	log_dir := home_dir
	if log_dir != default_dir() {
		test.Errorf("default_log_dir not returning expected directory. Expected: %s\tReceived: %s", log_dir, default_dir())
	}
}
...
func TestDefaultConfig(test *testing.T) {
	...
	if !cmp.Equal(d_config, *defaultCfg) {
		test.Errorf("default_config not returning expected config. Expected: %v\tReceived: %v", d_config, default_config())
	}
}
